...
async function debugJWTIssue() {
  ...
  try {
    ...
    if (response.ok) {
      console.log('✅ SUCCESS: JWT_SECRET is properly configured!');
      ...
      if (data.error === 'Authorization header required') {
        console.log('❌ JWT_SECRET is NOT set in Vercel');
        console.log('🔧 Action: Set JWT_SECRET environment variable');
  ...
  console.log('1. Verify JWT_SECRET is set in Vercel Dashboard');
