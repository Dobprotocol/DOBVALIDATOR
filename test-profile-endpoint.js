...
async function testProfileEndpoint() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('✅ JWT_SECRET is properly configured in backend');
    } else {
      console.log('❌ ERROR: Profile endpoint failed');
      if (data.error === 'Authorization header required') {
        console.log('❌ JWT_SECRET is NOT set in backend');
        console.log('🔧 DevOps needs to set JWT_SECRET environment variable');
      }
    }
