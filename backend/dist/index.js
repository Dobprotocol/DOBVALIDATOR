...
app.post('/api/auth/verify', async (req, res) => {
    try {
        ...
        let user = await database_2.userService.getByWallet(walletAddress);
        if (!user) {
            user = await database_2.userService.findOrCreateByWallet(walletAddress);
        }
        ...
        const expiresIn = env_validation_1.env.JWT_EXPIRES_IN || '7d';
    ...
    try {
        ...
        if (adminNotes !== undefined || adminScore !== undefined || adminDecision !== undefined) {
            ...
            if (adminNotes !== undefined)
                adminReviewData.notes = adminNotes;
            if (adminScore !== undefined)
                adminReviewData.overallScore = adminScore;
            ...
            await database_2.adminReviewService.upsert(id, adminReviewData);
        }
        const submission = await database_2.submissionService.update(id, submissionUpdates);
        res.json({ success: true, submission });
        return;
    }
    catch (error) {
...
app.get('/api/drafts', async (req, res) => {
    try {
        ...
        const drafts = await database_1.prisma.draft.findMany({
            where: { userId: user.id },
            orderBy: { updatedAt: 'desc' },
            take: parseInt(limit),
            skip: parseInt(offset)
        });
        const total = await database_1.prisma.draft.count({
            where: { userId: user.id }
        });
        res.json({
            success: true,
...
app.post('/api/drafts', async (req, res) => {
    try {
        ...
        const draft = await database_1.prisma.draft.create({
            data: {
                userId: user.id,
                ...draftData
            }
...
app.put('/api/drafts/:id', async (req, res) => {
    try {
        ...
        const existingDraft = await database_1.prisma.draft.findFirst({
            where: {
                id: id,
                userId: user.id
            }
        ...
        const updatedDraft = await database_1.prisma.draft.update({
            where: { id: id },
            data: {
                ...updateData,
                updatedAt: new Date()
...
app.delete('/api/drafts/:id', async (req, res) => {
    try {
        ...
        await database_1.prisma.draft.delete({
            where: { id: id }
        });
